...
static std::string qualifier_name(int q) {
  switch (q) {
...
void p6::xml_writer::do_takum3_node(cdk::takum3_node * const node, int lvl) {
  os() << std::string(lvl, ' ') << "<takum3_node>" 
       << node->value().to_string() << "</takum3_node>" << std::endl;
}
...
void p6::xml_writer::do_print_node(p6::print_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " newline=\"" << std::boolalpha << node->newline() << "\">" << std::endl;
...
void p6::xml_writer::do_block_node(p6::block_node * const node, int lvl) {
  ...
  if (node->statements()) node->statements()->accept(this, lvl + 4);
...
void p6::xml_writer::do_variable_declaration_node(p6::variable_declaration_node * const node, int lvl) {
  ...
  os() << std::string(lvl, ' ') << "<" << node->label() << " name='" << node->name() << "' qualifier='"
      << qualifier_name(node->qualifier()) << "' type='" << (node->type() ? node->type()->to_string() : "auto") << "'>" << std::endl;
...
void p6::xml_writer::do_function_node(p6::function_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
...
void p6::xml_writer::do_alloc_node(p6::alloc_node * const node, int lvl) { 
  ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  node->argument()->accept(this, lvl + 2);
  closeTag(node, lvl);
...
void p6::xml_writer::do_function_declaration_node(p6::function_declaration_node * const node, int lvl) { 
  ...
  os() << std::string(lvl, ' ') << "<" << node->label() << " name='" << node->name() << "' qualifier='"
      << qualifier_name(node->qualifier()) << "' type='" << (node->type() ? node->type()->to_string() : "?") << "'/>" << std::endl;
