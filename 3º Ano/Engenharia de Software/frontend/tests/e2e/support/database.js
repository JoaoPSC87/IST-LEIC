...
const threeDaysAfterTomorrow = new Date(now);
threeDaysAfterTomorrow.setDate(now.getDate() + 4);
...
Cypress.Commands.add('createDatabaseInfoForEnrollments', () => {
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(1, "A1", "Enrollment is open", tomorrow.toISOString(), tomorrow.toISOString(),
      threeDaysAfterTomorrow.toISOString(), 5, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
...
function syncShiftIdSequence() {
  cy.task('queryDatabase', {
    query: "SELECT setval('shift_id_seq', COALESCE((SELECT MAX(id) FROM shift), 1))",
    credentials: credentials,
  })
}
