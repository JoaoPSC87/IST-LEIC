describe('Participation', () => {
  ...
  it('create participation', () => {
    ...
    // check that one enrollment is not yet participating
    cy.get('[data-cy="activityEnrollmentsTable"] tbody tr')
  ...
  it('shows error when participants limit of shift is exceeded', () => {
    const MEMBER_REVIEW = 'The volunteer did a good job';

    cy.intercept('GET', '/activities/2/enrollments').as('enrollments');
    cy.intercept('POST', '/participations/*/enrollment/*').as('participation');
    ...
    // Open enrollments for activity A2 (index 1) - shift is already at capacity (limit=1)
    cy.get('[data-cy="memberActivitiesTable"] tbody tr')
      .eq(1)
      .find('[data-cy="showEnrollments"]').click()
    cy.wait('@enrollments');

    // Try to create participation - shift is already full
    ...
    cy.wait('@participation').then(({ response }) => {
      expect(response.statusCode).to.eq(400)
      expect(response.body.message).to.contain('Current participants cannot exceed participants limit')
    })

    cy.contains('.v-alert', 'Current participants cannot exceed participants limit').should('be.visible')
