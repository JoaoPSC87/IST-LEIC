...
export default class InstitutionActivitiesView extends Vue {
  ...
  getShiftsCapacity(activity: Activity) {
    const shiftsCapacity = activity.shifts
      ? activity.shifts.reduce((sum, shift) => sum + shift.participantsLimit, 0)
      : 0;
    return `${shiftsCapacity}/${activity.participantsNumberLimit}`;
  }
