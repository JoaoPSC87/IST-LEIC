<template>
  <v-card class="table">
    ...
    <participation-selection-dialog
      ...
      :enrollment-id="currentEnrollment ? currentEnrollment.id : null"
      :available-shifts="availableEnrollmentShifts"
...
export default class InstitutionActivityEnrollmentsView extends Vue {
  ...
  async selectParticipant(enrollment: Enrollment) {
    ...
    if (existingParticipation) {
      ...
      this.currentParticipation.activityId = activityId;
      this.currentParticipation.volunteerId = volunteerId;
      if (this.availableEnrollmentShifts.length > 0) {
        this.currentParticipation.shiftId = this.availableEnrollmentShifts[0].id;
      }
      this.editParticipationSelectionDialog = true;
    } else {
      const selectedShiftId = this.availableEnrollmentShifts[0]?.id;
      if (enrollment.id !== null && selectedShiftId !== null && selectedShiftId !== undefined) {
        this.createParticipation(activityId!, volunteerId!, enrollment.id, selectedShiftId);
      }
    }
  }

  get availableEnrollmentShifts(): Shift[] {
    if (!this.currentEnrollment) return [];
    return this.shifts.filter(
      (shift) => shift.id !== null && this.currentEnrollment!.shiftIds.includes(shift.id),
    );
  }
  ...
  async createParticipation(
    ...
    this.currentParticipation.activityId = activityId;
    this.currentParticipation.volunteerId = volunteerId;
    this.currentParticipation.shiftId = shiftId;
