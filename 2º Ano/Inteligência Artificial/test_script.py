...
def run_solver_test(test_number):
    ...
    test_dir = base_dir / "tests"  # <-- Corrige aqui
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, str(base_dir / "nuruomino.py")],
            input=input_data,
            text=True,
            capture_output=True,
            timeout=6,
        ...
        return f"Test {test_number:02d}: FAILED - Timeout (>100s)", 100.0
...
def main():
    ...
    test_files = list(test_dir.glob("test-*.txt"))
    test_numbers = []

    for test_file in test_files:
        try:
            # Extract number from filename like "test-01.txt"
            test_num = int(test_file.stem.split("-")[1])
    ...
    if not test_numbers:
        ...
        return print(f"Found {len(test_numbers)} test cases: {test_numbers}")
    print("-" * 60)
