...
function createTargets(target_size, horizontal_gap, vertical_gap)
{
  ...
  for (let i = 0; i < legendas.getRowCount(); i++) {
    items.push({ id: legendas.getNum(i, 0), label: legendas.getString(i, 1) });
  }
   items.sort((a, b) => a.label.localeCompare(b.label));

  // Mark first target of each letter group
  let currentLetter = '';
  for (let i = 0; i < items.length; i++) {
    let firstLetter = items[i].label.charAt(0).toUpperCase();
    items[i].isFirstOfGroup = (firstLetter !== currentLetter);
    if (items[i].isFirstOfGroup) currentLetter = firstLetter;
  }

  // Set targets in a 8 x 10 grid (alphabetical order)
  for (var r = 0; r < GRID_ROWS; r++)
  {
    for (var c = 0; c < GRID_COLUMNS; c++)
    {
      ...
      let idx = c + GRID_COLUMNS * r;
      let target = new Target(target_x, target_y, target_size, items[idx].label, items[idx].id, items[idx].isFirstOfGroup);
      targets.push(target);
    }
  }
}
...
function windowResized() 
{
  if (fullscreen())
  {
    ...
    createTargets(target_size * PPCM, horizontal_gap * PPCM - 80, vertical_gap * PPCM - 120);
